...
by CI on every push (Figure 25). The full plan is Appendix A; this section gives
...
test double; the MySQL rules against a real MySQL 8 server, because the point of
a trigger is that it holds *inside the transaction*; the presentation tier
...
rule, `ffa86d5` → `8b73b14` for the discounts (Figure 29). Checking out a red
...
new because they share a class. The partitions are tabulated in Appendix A.
**Boundary value analysis** then tests each edge at the value and at the
adjacent value either side, because every failure the clinic reported is at an
edge.

The boundaries tested — opening and closing times, the fifteen-minute grain, the
ten-minute buffer, age 65, the fifth visit — are tabulated in Appendix A, each
at the value and the adjacent value either side.
...
the **consequence** — the appointment exists, the status changed, the audit
trail records it. Fowler (2007) frames this as behaviour versus state
...
| FR-20, 24 notification and audit | `NotificationObserver`, `AuditLogObserver` | UT-APPT-03, 04; Figs 23, 24 |
| FR-21, 22 maintain treatments and staff | `AdministrationService`, admin endpoints | UT-ADM-01…18; Figs 19, 20 |
...
| NFR-07, 09 automation and usability | `ci.yml`; browser UI | Figs 27, 28; Figs 11–14 |

Every requirement traces to the object that meets it and the test or figure that
proves it. Per-requirement rows and the full plan are in Appendix A.
...
Line coverage is 47.5% and branch coverage 73.8% (Figure 26). The headline is
